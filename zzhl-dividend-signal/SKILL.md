...
7. 理杏仁 Open API 成功返回后，将近10年分位数据缓存到 `sources/manual/中证红利信号/理杏仁分位缓存.json`。

`最新信号.md` 的判断区必须使用三列表格：
...
- 理杏仁返回 HTTP 429 Too Many Requests 时，不再继续请求公开页面；脚本会优先使用本地缓存，若没有可用缓存，则将历史分位点标记为待验证，避免加重限流。
